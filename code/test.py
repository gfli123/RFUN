...
from build_models import  Image_VSR
from img_preprocess import imread, img_trans, modcropHR, img_normal, bgr2ycbcr, modcrop
    ...
    for data_test in test_datasets:
        ...
        for test_name in test_list:
            ...
            for i in range(0, len(inList)):
                img = imread(inList[i])
                HR_all.append(img)
            HR_all = [modcrop(HR, opt.scale) for HR in HR_all]
            HR_all = [trans_tensor(HR).float() for HR in HR_all]
            HR_all = torch.stack(HR_all, dim=1)

            LR = Guassian_downsample(HR_all, opt.scale)          # LR image
            LR = LR.unsqueeze(0)
            ...
            with open('./results/time_all.txt', 'a+') as f:
                f.write('times = {:.6f}'.format(cost_time / len(inList)) + '\n')
...
if __name__ == '__main__':
    ...
    path_checkpoint = "./X4_epoch_90.pth"
